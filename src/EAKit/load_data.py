...
class AlignmentData:
    ...
    def __init__(self, ent_ids, rel_ids, train_pair, dev_pair, test_pair, triples,
                 share=False, swap=False, val=0.0, with_r=False, **kwargs):
        t_ = time.time()

        # self.rate = rate
        self.val = val
        self.kg1_ins_ids, self.kg2_ins_ids = ent_ids
        self.kg1_rel_ids, self.kg2_rel_ids = rel_ids
        self.ins_num = len(ent_ids[0]) + len(ent_ids[1])
        self.rel_num = len(rel_ids[0]) + len(rel_ids[1])
        # np.random.shuffle(self.ill_idx)
        self.triple_idx = triples
        self.ill_train_idx, self.ill_val_idx, self.ill_test_idx = train_pair, dev_pair, test_pair
        self.ill_idx = np.concatenate([self.ill_train_idx, self.ill_val_idx, self.ill_test_idx], axis=0)
